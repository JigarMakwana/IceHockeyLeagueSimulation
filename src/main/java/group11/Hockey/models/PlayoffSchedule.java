	...
	public HashMap<String, HashMap<Team, Team>> generatePlayoffScheduleRound1(String date){
		...
		for (Conference conference : cconferenceList) {	
			...
			for (Division division : divisionList) {
				firstHighestPoints=0;secondHighestPoints=0;thirdHighestPoints=0;
				firstHighestTeam=null;secondHighestTeam=null;thirdHighestTeam=null;tempTeam=null;
			...
			firstHighestPoints=0;secondHighestPoints=0;
			firstHighestTeam=null;secondHighestTeam=null;tempTeam=null;
			for (Division division : divisionList) {
				List<Team> teamList = division.getTeams();
				for (Team team : teamList) {
					if((team==roundOne.get(0))||(team==roundOne.get(1))||(team==roundOne.get(2))||(team==roundOne.get(4))||(team==roundOne.get(5))||(team==roundOne.get(6))) {
						continue;
					}
					else {
			...
			while(teams<4){							//four final sets of teams(team1,team2) from each conference - FIRST ROUND
				...
				while(series<7){
					...
					System.out.println("Scheduled b/w "+team1.getTeamName()+" & "+team2.getTeamName()+" on "+date+" at "+time);
